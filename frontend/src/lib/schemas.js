...
export const classroomSchema = z.object({
    ...
    joinCode: z.union([
        z.literal(""),
        z.string().min(6, "Join code must be at least 6 characters"),
    ]),
    subjectId: z.preprocess(
        (val) => (val === null || val === "" ? undefined : Number(val)),
        z.number({ required_error: "Subject is required", invalid_type_error: "Subject is required" }).min(1, "Subject is required"),
    ),
});

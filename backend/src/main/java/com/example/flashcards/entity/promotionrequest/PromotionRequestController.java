    ...
    public ResponseEntity<ApiResponse<PromotionRequestResponse>> updateRequest(
        ...
        PromotionRequest promotionRequest = switch (request.status()) {
            ...
            default -> throw new InvalidRequestException(
                "error.promotionRequest.invalidStatus",
                new Object[]{request.status()}
            );
        };
        return ResponseEntity.ok(ApiResponse.success(
            PromotionRequestResponse.from(promotionRequest),
            messageSource.getMessage("success.promotionRequest.updated", null, locale)
        ));
    }
}
